...
curl -N -X POST https://${new URL(request.url).host}/v1/chat/completions \\
...
const response = await fetch('https://${new URL(request.url).host}/v1/chat/completions', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
